...
- `adapter` — `theme.toml` `[adapters.<tool>]` as written (empty table if absent)
  - `adapter.mirror` is optional for every tool, validated as `<owner>/<repo>` before rendering (`Error::AdapterMirror`), and README templates branch only on its presence.
